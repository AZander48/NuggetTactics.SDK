...
{
    public const uint APP_ID = HadeanTacticsVersionInfo.STEAM_APPID;
    ...
    public async Task<HadeanTacticsVersionInfo> GetProductInfoAsync()
    {
        ...
        ulong token = tokenResult.AppTokens.TryGetValue(APP_ID, out ulong t)
            ? t
            : throw new SteamClientException("Could not get PICS token for Hadean Tactics, do you own the game?");
        ...
        if (appInfo == null)
        {
            throw new SteamClientException("Steam did not return any app info for Hadean Tactics.");
        }
        LogKeyValues(appInfo.KeyValues);

        KeyValue depots = appInfo.KeyValues["depots"];
        ulong winManifestId = depots[HadeanTacticsVersionInfo.STEAM_DEPOT_ID_WINDOWS.ToString()]["manifests"]["public"]["gid"].AsUnsignedLong();
        ulong macManifestId = depots[HadeanTacticsVersionInfo.STEAM_DEPOT_ID_MAC.ToString()]["manifests"]["public"]["gid"].AsUnsignedLong();
        ulong linuxManifestId = depots[HadeanTacticsVersionInfo.STEAM_DEPOT_ID_LINUX.ToString()]["manifests"]["public"]["gid"].AsUnsignedLong();
        return new()
        {
            Version = "UNKNOWN",
