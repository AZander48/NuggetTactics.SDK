...
partial class Build : NukeBuild
{
    ...
    [Parameter("Target NuggetTactics versions",
        ValueProviderType = typeof(HadeanTacticsVersionInfo),
        ValueProviderMember = nameof(HadeanTacticsVersionInfo.AllVersionStrings))]
    readonly HadeanTacticsVersionInfo[] TargetHadeanTacticsVersions;
    ...
    Target Package => _ => _
        .Description("Builds the SDK NuGet package (props + readme only; no game files).")
        .Executes(() =>
        {
            Log.Information("Packaging NuggetTactics.SDK");
            if (DryRun)
            {
            ...
            DotNetTasks.DotNetPack(_ => _
                .SetProject(Solution.NuggetTactics_SDK.RelativePath)
                .SetConfiguration("Release")
                .SetProperty("BaseIntermediateOutputPath", $"{ObjDir}/")
                .SetProperty("BaseOutputPath", $"{BinDir}/")
            );
        });

    Target Publish => _ => _
        .Description("Publishes the SDK NuGet package with guardrails.")
        ...
        .Executes(() =>
        {
            ...
            IEnumerable<AbsolutePath> packages = BinDir.GlobFiles("**/*.nupkg");
    ...
    Target DownloadDepots => _ => _
        .Description("Downloads game files locally for maintainer experiments. Not used by the published SDK package.")
        .Executes(async () =>
        {
            IEnumerable<HadeanTacticsVersionInfo> targetVersions = GetTargetVersions();
            ...
            uint depotId = HadeanTacticsVersionInfo.STEAM_DEPOT_ID_WINDOWS;
            ...
            await Parallel.ForEachAsync(targetVersions, new ParallelOptions { MaxDegreeOfParallelism = 8 }, async (v, ct) =>
            {
                ...
                foreach (DepotManifest.FileData file in validFiles)
                {
                    ...
                    {
                        string relativePath = Path.GetRelativePath("HadeanTactics_Data/Managed", Path.TrimEndingDirectorySeparator(file.FileName));
    ...
    Target GetCurrentVersionInfo => _ => _
        .Description("Uses Steam to get the version info (manifest info and game version) for the latest version of Hadean Tactics")
        .Executes(async () =>
        {
            SteamClientWrapper clientWrapper = new();
            await clientWrapper.ConnectAndLoginAsync(SteamUser, SteamPassword);

            HadeanTacticsVersionInfo versionInfo = await clientWrapper.GetProductInfoAsync();
            ...
            DepotManifest.FileData ggm = manifest.Files.FirstOrDefault(f => f.FileName.Replace('\\', '/').Equals("HadeanTactics_Data/globalgamemanagers"));
            ...
            HadeanTacticsVersionInfo existingRecord = HadeanTacticsVersionInfo.AllVersions.FirstOrDefault(v => v.Version == gameVersion);
    ...
    private IEnumerable<HadeanTacticsVersionInfo> GetTargetVersions()
    {
        return TargetHadeanTacticsVersions != null && TargetHadeanTacticsVersions.Length > 0
            ? TargetHadeanTacticsVersions
            : HadeanTacticsVersionInfo.AllVersions;
    }
    ...
    [GeneratedRegex(@"^HadeanTactics_Data/Managed/.+$")]
